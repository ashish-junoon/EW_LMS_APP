...
import { DynamicData } from '../../api/ApiFunction';
...
const DynamicReporting = () => {
    ...
    const formik = useFormik({
        ...
        validationSchema: Yup.object({
            ...
            source: Yup.string().when('dataType', {
                is: (dataType) => Boolean(dataType),
        ...
        onSubmit: async ({ fromDate, toDate, dataType, source }) => {
            ...
            try {
                const response = await DynamicData(req);
                if (response.status) {
                    setIsLoading(false);
                    setReportData(response.reportDetail);
    ...
    return (
        <>
            <span className="md:text-lg italic font-semibold bg-primary text-white rounded-t-lg px-5 py-1">Dynamic Report </span>
            <div className="border border-gray-200 rounded shadow-sm">
                ...
                <form onSubmit={formik.handleSubmit} className="p-2 md:p-6 space-y-6">
                    ...
                    <div className="grid md:grid-cols-4 gap-2 md:gap-5">
                        <div className='max-md:col-span-4 max-lg:col-span-2'>
                            <SelectInput
                                label="Data Type"
                        ...
                        {/* Source Field - CORRECTED THIS SECTION */}
                        <div className='max-md:col-span-4 max-lg:col-span-2'>
                            <SelectInput
                                label="Data Source"
                                icon="RiDatabase2Line"
                                ...
                                disabled={!formik.values.dataType}
                            />
                            {formik.touched.source && formik.errors.source && (
                                <ErrorMsg error={formik.errors.source} />
                            )}
                        </div>
