...
const LeadQCForm = () => {
  ...
  const handleApproveYes = () => {
    const req = {
      ...
      remarks:
        "Documents & information have been verified and sent for Credit.",
    };
  ...
  const fetchLoanData = async ({ customerId }) => {
    setIsLoading(true);
    const req = {
    ...
    try {
      ...
      if (response.status && response.customerLoanHistories) {
        // localStorage.setItem("customerId", JSON.stringify({customerId: customerId}))
        sessionStorage.setItem(
          "customerId",
          JSON.stringify({ customerId: customerId }),
        );
        const url = `/admin/loan-history`;
        // const url = `/admin/loan-history?customerId=${customerId}`;
      ...
      content: (
        <div>
          ...
          <div className="grid lg:grid-cols-7 gap-4 mt-5">
            ...
            >
              <Personal btnEnable={!isOnHold && permission} />
              <Employment btnEnable={!isOnHold && permission} />
              <Address btnEnable={!isOnHold && permission} />
              <KycInfo btnEnable={!isOnHold && permission} />
              <BankInfo btnEnable={!isOnHold && permission} />
              <Gaurantor btnEnable={!isOnHold && permission} />
              <OthersDocs btnEnable={!isOnHold && permission} />

              {userData?.lead_status === 2 && permission && (
                <div className="col-span-2">
                  <div className="flex justify-end gap-5">
                    {leadInfo?.personal_info_verified &&
                      leadInfo?.employment_info_verified &&
                      leadInfo?.address_info_verified &&
                      leadInfo?.kyc_info_verified &&
                      leadInfo?.bank_info_verified &&
                      leadInfo?.gurantor_nominee_verified && (
                        <Button
