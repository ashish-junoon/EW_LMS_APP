...
import { DisbursmentCollectionReport_V1 } from "../../api/ApiFunction";
import { toast } from "react-toastify";
import { Loader2 } from "lucide-react";
import { useAuth } from "../../context/AuthContext";

const DisbursementCollectionReportV1 = () => {
  // Data type options
  ...
  const formik = useFormik({
    ...
    onSubmit: async ({ fromDate, toDate, dataType, source }) => {
      ...
      try {
        const response = await DisbursmentCollectionReport_V1(req);
        if (response.status) {
          setIsLoading(false);
...
export default DisbursementCollectionReportV1;
