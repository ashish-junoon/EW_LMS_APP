...
import { MasterSheetsReport } from "../../api/ApiFunction";
import { toast } from "react-toastify";
import { Loader2 } from "lucide-react";
import { useAuth } from "../../context/AuthContext";

const LeadReportStatusWise = () => {
  // Data type options
  ...
  const formik = useFormik({
    ...
    onSubmit: async ({ fromDate, toDate, dataType, source }) => {
      ...
      try {
        const response = await MasterSheetsReport(req);
        if (response.status) {
          setIsLoading(false);
...
export default LeadReportStatusWise;
